...
| `BRIDGE_ENCRYPT_KEY` | 空(纯明文) | **非空 = 开启「加密必需」模式**:会话内容经 HKDF→AES-256-GCM 端到端加密,隧道内不可读;**App 档案需配置同一密钥**(见 docs/03 §5)。设 `auto` = 首次启动自动生成并打印,持久化到 `bridge/.encrypt-key`(0600,重启复用,`cat` 可查看) |
